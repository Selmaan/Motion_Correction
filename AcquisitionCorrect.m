function newmov=AcquisitionCorrect(mov,xoffsets,yoffsets,fignum)
...
for i=1:z
    ...
    %coordinates
    thisframe_interp=interp2((1:x)+xoffsets(i),((1:y)+yoffsets(i))',thisframe,1:x,(1:y)','*linear', mean(mean(thisframe)));
   
    %save the result, clipping the edges as neccesary
    %newmov(:,:,i)=thisframe_interp(1+maxyshift:end+minyshift,1+maxxshift:end+minxshift);
    newmov(:,:,i) = thisframe_interp;
